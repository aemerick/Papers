...
\begin{abstract}
Galactic chemical evolution is driven through stellar feedback and its ability to drive metal mixing within the interstellar medium (ISM) and beyond the galaxy in outflows. However, we are far from a complete understanding of feedback along and galactic chemodynamics in general. In order to better understand this relationship, we have developed a new model for stellar feedback and galactic chemical evolution at high resolution in the adaptive mesh refinement hydrodynamics code \textsc{Enzo}. For the first time in galaxy scale simulations, we simulate detailed stellar feedback from individual stars including stellar winds from massive and asymptotic giant branch stars, photoelectric heating, Lyman-Werner dissociation, ionizing radiation tracked through an adaptive ray-tracing radiative transfer method, and core collapse and Type Ia supernovae. 
...
\begin{enumerate}
\item Hot Ionized Medium (\ion{H}{1}M): $T \geq 10^{5.5}$ K
