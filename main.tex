...
\title{Variations in the Distribution and Evolution of Individual Metals in Dwarf Galaxies}
...
\begin{abstract}
Using a high resolution simulation of an isolated dwarf galaxy, accounting for stellar feedback and chemical evolution on a star-by-star basis, we investigate how each of 15 metal species are distributed within our multi-phase interstellar medium (ISM) and ejected from our galaxy through galactic winds. For the first time, we demonstrate that the mass fraction probability distribution functions (PDFs) of individual metal species in the ISM are well described by a piecewise log-normal + power-law distribution. The log-normal component generally describes gas at lower metal fractions undergoing enrichment towards the mean metal fraction, driven by recent enrichment events in the power-law tail. The PDF properties, and relative significance of these two components, vary within each ISM phase. Hot gas is dominated by recent enrichment, with a significant power-law tail to high metal fractions, while cold gas is dominated by the log-normal component. In addition, elements dominated by asymptotic giant branch (AGB) wind enrichment (such as N and Ba) mix less efficiently than elements dominated by supernova enrichment (such as $\alpha$ elements and Fe). This difference is driven by the differences in energetics between each source, particularly the poor coupling of AGB winds. Only 60\% of metals dominated by AGB enrichment are ejected through winds, even though 90\% of all metals leave the galaxy. In dwarf galaxies, therefore, elements synthesized predominately through AGB winds should be both overabundant and have a larger spread than elements synthesized in either core collapse or Type Ia supernovae. We discuss the observational implications of these results, their potential use in developing improved models of galactic chemical evolution, and their generalization to more massive galaxies.
\end{abstract}

\keywords{}

\section{Introduction}
% can I work "Currents of Space" into this paper somehow (Asimov)?
Understanding galactic chemical evolution for all metal species across galaxy mass scales remains one of the most challenging aspects of modeling galaxy evolution (review refs for obs and theory). One of the most pressing difficulties is a lack of understanding in exactly how metals propagate from their injection sites (e.x. stellar winds or supernovae) and mix through the phases of the interstellar medium (ISM) into star forming gas. (\textit{... more...})
% Discuss common modeling assumptions in CE models that
%    can be addressed here:
%        1) instantaneous recycling approximation
%        2) No metal mass loading of winds, or uniform 
%           loading across species
%        3) Uniform metal abundances across the ISM (one zone)
%
%  To some degree, cosmological simulations with comparably
%     low spatial resolution face the same problems, where
%     variations and spreads in abundances (esp. across 
%     species) can be smoothed over due to the low res. or
%     the particular details of the feedback methods and
%     yield injection methods. 

One-zone chemical evolution models often assume instantaneous mixing of metals from recent star formation into gas available for future star formation (references). While this incorrectly assumes that metal mixing in the ISM plays no role in delaying future enrichment in star formation, how to properly account for this process is poorly understood. This is in large part because both these models and large-scale cosmological simulations lack the necessary fidelity to capture the detailed, multi-phase mixing process of metals in the ISM directly. Recent hydrodynamics simulations have employed parameteric models to account for unresolved sub-grid metal mixing (references), which plays an important role in determining the chemical properties of galaxies (references). % more on mixing
Consequently, following metal mixing, if treated in these models, requires assumptions in how metals mix within the ISM and how metals couple to galactic outflows. When metals are tracked individually, as opposed to a global metallicity field, their injection, mixing, and outflow properties are often treated uniformly. However, these processes are critical components in understanding galactic chemical evolution. Detailed hydrodynamics simulations incorporating a realistic multi-phase ISM and detailed stellar feedback are required to understand metal mixing and metal outflows. In addition, it remains to be seen to what degree, if at all, metals of different nucleosynthetic origins may couple differently to the ISM. 

The use of low mass dwarf galaxies, both observationally in the Local Group and in theoretical models, has been critical in improving our understanding of galactic chemical evolution. (\textit{... more...}) That dwarf galaxies are efficient metal polluters has been demonstrated for some time both theoretically \citep[e.g.][]{DekelSilk1968,MacLowFerrara1999} with direct observational evidence found in the metal retention fractions of Local Group dwarf galaxies \citep[e.g.][]{Kirby2011-metals,McQuinn2015}. It is commonly assumed that all metals couple equally to galactic outflows, however, which may not be a valid assumption if metals of different nucelosynthetic origins couple differently to the ISM \citep{KrumholzTing2018}. Relaxing this assumption has implications for both interpreting observations of stellar abundances in nearby dwarf galaxies and in modeling galactic chemical evolution in semi-analytic models and lower resolution cosmological simulations. However, testing this assumption has only become possible recently as it requires high resolution, galaxy-scale hydrodynamics simulations that can resolve ISM mixing and self-consistently drive galactic winds through multi-channel stellar feedback. 

% Paragraphs
It is becoming increasingly important to develop a concrete theoretical understanding of galactic chemical evolution as a result of multiple, recent observational campaigns to probe detailed stellar abundances in our Milky Way and the Local Group. (APOGEE, GAIA, GALAH, RAVE? TGAS?). Stellar abundances are directly imprinted with the enrichment pattern of their star forming cloud, whose chemical properties are determined by the process of turbulent metal mixing in the ISM. The degree to which we can associate "chemically tagged" stars as co-eval depends directly upon our understanding of metal enrichment and metal mixing in the ISM. However, this understanding is critical for using these observations to further deduce properties of a galaxy's evolutionary history.

% Can shorten if this is actually going to be a letter
...
%\begin{figure}
%\centering
%\includegraphics[width=0.95\linewidth]{sfr_snrx100_agb.png}\\
%\caption{}
%\label{fig:sfr}
%\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth]{species_bar.png}\\
\includegraphics[width=0.95\linewidth]{species_bar_ISM.png}
\caption{The fraction of each metal species in the full simulation box contained in the halo, gas in the galaxy disk, and stars (top), and the fraction of species within the disk alone in each phase of the ISM (bottom). The leftmost bar in each plot shows the sum of all metals. \textbf{font sizes}}
...
The top panel of Figure~\ref{fig:species_fractions} gives the mass fraction of each element at the end of the simulation (500~Myr) in each of four reservoirs: locked in stars (yellow), in the ISM (blue), outside the galaxy but within the virial radius (orange), and outside the virial radius (green). Gas that has left the simulation domain is included in this final category. We subdivide the ISM by phase in the bottom panel, giving the mass fractions of each element in molecular gas ($f_{\rm H_2} > 0.5$, $T < 100 K$), the cold neutral medium (CNM, $f_{\rm H_2} < 0.5$,  T$< 100$~K), warm neutral medium (WNM, $10^2~\rm{K}\le \rm{T} < 10^4~\rm{K}$), warm ionized medium (WIM, $10^4~\rm{K}\le \rm{T} < 10^{5.5}~\rm{K}$), the hot ionized medium (HIM T$\ge 10^{5.5}$~K), and locked in stars (yellow). Again, with the exception of H and He, we only consider metals produced self-consistently through our star formation and stellar feedback methods; the initial mass of each metal species is zero.

We find two major results. First, most of the metals in the ISM reside in cold gas, either the CNM or molecular gas, with only a few percent in the hot phases. The exact fraction varies with each species, but only on the level of a few percent. This is not surprising, as the cold phases represent the majority of the mass in the ISM, but, as shown in Section~\ref{sec:statistics}, even though the cold phases harbor most of the metals, the hot phases have higher metal mass fractions. Second, only a small fraction of produced metals are retained within the dwarf galaxy, in agreement with observations of nearby dwarf galaxies \citep[see][]{Kirby2011-metals, McQuinn2015}; however,the retention factor varies. The top panel shows a qualitative disagreement between the retention fractions of N and Ba (about 40\% for each) as compared to the rest of the metals ($\sim$ 10\%). This suggests that individual metals \textit{do not} share the same dynamical evolution. Clearly, metal enrichment in galaxies is a phenomenon that cannot be fully captured using single, global metallicity field. In this particular case, assuming that all metals behave the same would underestimate the galactic N and Ba enrichment by a factor of four.
...
The only physics that separates the dynamical evolution of these elements in our simulations is the individual sources of enrichment: AGB winds, stellar winds (from stars above 8~M$_{\odot}$), core collapse supernovae, and Type Ia supernovae. These channels differ in how long after a given star formation event they occur, and by consequence the typical ISM properties in which they occur, and how much energy is associated with each event, which determines the temperature of the gas carrying each element. To understand where each of the elements in Figure~\ref{fig:species_fractions} originated, we show the mass fraction of metals produced through each channel in Figure~\ref{fig:species_sources}. Core collapse supernovae are responsible for over 90\% of the total metal enrichment in our galaxy, but this is clearly not true for all elements. In particular, a majority of N (76\%) and Ba (65\%) are ejected by AGB winds, and both are retained at a higher rate than the rest of the metals. That this behavior exists for N and Ba in our simulations is dependent upon the metallicity of our galaxy and choice of stellar yields tables, but we can generalize this result to say that, for any assumed set of yields, low mass galaxies should more easily retain \textit{any} elements synthesized predominately in AGB winds, as compared to elements synthesized through supernovae.\footnote{One might expect that much of the N and Ba that is ejected by galactic winds is comprised mostly of the $\sim$30\% of each species produced through the stellar winds of more massive stars and supernovae. However, this cannot be determined as we lack Lagrangian information about individual gas elements.} We discuss how this result may extend towards other metal yields that are not well sampled on our relatively short (500 Myr) simulation timescales in Section~\ref{sec:discussion:metal yields}.

AGB winds have low energy and velocities (10~km~s$^{-1}$) as compared to the energy and typical expansion velocities of supernovae ($\sim$1000~km~s$^{-1}$). In addition, their longer timescales (40-100~Myr as compared to $\sim$10~Myr) means that AGB stars are typically removed from their birth regions. The changes in typical ISM density and height of these events contributes to these variations. We show histograms of the average number density within 20~pc of the source (top) and height above/below the disk (bottom) within 1~Myr of the event (as limited by our output cadence) in Figure~\ref{fig:spatial distribution}. As shown, SN have a bi-modal distribution in density as they explode either close to their dense formation region or within superbubbles, while AGB stars are singly peaked between this distribution at close to the average ISM density. We do not expect these differences to be the dominant effect in driving the differential evolution of elements ejected by AGB winds vs. supernovae, compared to the energetics.

\begin{figure}
\centering
\includegraphics[width=0.95\linewidth]{SE_density.png}\\
\includegraphics[width=0.95\linewidth]{SE_z.png}
\caption{The average gas number densities within 20~pc of a given event (top) and vertical position above/below the disk (bottom) within 1~Myr before the event. {\bf ignore SNIa line in plot}}
...
\end{cases}
\end{align*}
where $N$ is a normalization constant, $p_o$ ensures continuity between the two components, $\mu$ and $\sigma$ are the log-mean and width of the log-normal component, $\alpha$ is the power-law slope,  and Z$_{\rm trans}$ is the metal fraction at the transition between the log-normal and power-law components. When fitting this PDF, we only enforce continuity and $\int_0^{\infty} p(Z) dZ = 1$, leaving $\alpha$, $\mu$, $\sigma$, and $Z_{\rm trans}$ as free parameters. These conditions set $N$ and $p_o$ to 
\begin{equation}
N = 
\end{equation}
and
\begin{equation}
p_o = \frac{1}{\sigma Z_{\rm trans} \sqrt{2 \pi}} {\rm exp}\left[-\frac{({\rm ln}(Z_{\rm trans}) - \mu)^2}{2 \sigma^2} + \alpha {\rm ln}(Z_{\rm trans}) \right]
\end{equation}.

We compute the numerical metal mass fraction PDFs for each metal species in our simulation using a fixed bin with of 0.05 dex. We fit $p(Z)$ to each of these using a Levenberg-Marquardt algorithm as implemented in \textsc{Scipy} \cite{Scipy}, stepping through possible values for $Z_{\rm trans}$, set to the centers of each of these bins. The best of these fits is then compared to best fits using only a log-normal component or only a power-law component, and the best of these three is accepted. We note that the log-normal + power-law PDF produced the best fit in nearly all cases. 

We show in \ref{fig:log-normal} the numerical PDFs (solid histograms) and log-normal + power-law fits (dashed lines) across individual gas phases. These PDFs have been computed at an arbitrary single point in time, at 380 Myr. For clarity, we only show a subset of the 15 elements we follow. As shown, there are clear differences in the PDFs across elements of different nucleosynthetic origins and between each phases. However, each of these distributions are characterized by a power-law tail towards high metal fractions and a turnover of varying width at low metal fractions. We discuss the differences among each phase in more detail in the next section, but note here that the significance of each of these components varies notably across phases. In many of these cases, the piecewise log-normal + power-law distribution fits the numerical PDF quite well. However, there are often deviations, particularly at low metal fractions, from pure log-normal behavior. This manifests as either a very broad, flat PDF at low metal fraction (see N and Ba, for example), or large peaks not well described by $p(Z)$ at low metal fraction. In these situations, it is unclear what, if any, portion should be considered as a log-normal, or if there are multiple components within this region.

We argue here that the log-normal + power-law PDF can be a powerful tool for modelling the the metal fraction  PDFs of individual elements in galaxy models. The fits are not uniformly perfect but some deviation from a simple analytic model is expected in a complex, multi-phase ISM. In addition, some of this deviation, particularly in the WNM and WIM, could be caused by grouping together qualitatively distinct gas in a single phase; this would tend to broaden the distributions. Most importantly, however, the PDFs of the coldest gas phases, the CNM and molecular gas, are indeed well fit by the adopted $p(Z)$. As this is the source of star forming gas in the ISM, the log-normal + power-law PDF appears useful to account for intrinsic scatter in stellar abundances in galaxy evolution models. 
%Developing a complete physical model for what drives the formation and properties of $p(Z)$ is beyond the scope of this paper, but these prospects are discussed in Section~\ref{sec:discussion}.

%The log-normal distribution is a two parameter model defined by the log mean $M$ and the log standard deviation $S$. These values are related to the linear mean, $\mu$, and standard deviation, $\sigma$, as $\mu = e^{M+S^2/2}$ and $\sigma = e^{S^2 + 2M}(e^{S^2}-1)$. We define the log-normal distribution of metals in the ISM as a function of either the total metallicity $Z$ or the mass fraction of an individual metal species, $Z_i$, where $i$ denotes an individual element. For clarity in this section, we refer most often to total metallicity as the variable of interest. However, we note that any statement that applies to metallicity also applies to the mass fraction an individual metal species ($Z_i$). With these definitions, the log-normal metallicity PDF is given as
%\begin{equation}
%f(Z_i, M_i, S_i) = \frac{1}{\sqrt{2 \pi} S_i} \rm{exp}\left(\frac{-(ln(Z_i) - M_i)^2}{2S_i^2}\right).
%\end{equation}
%We can also specify the CDF of this distribution as 
%\begin{equation}
%F(Z_i, M_i, S_i) = \frac{1}{2} \left[1 + \rm{erf} \left(\frac{ln(Z_i) - M_i}{S_i \sqrt{2}} \right). \right].
%\end{equation}
%Thus, $F(Z_i, M_i, S_i)$ gives the fraction of mass in the ISM with metallicity $Z < Z_i$. For each of the 15 metal species in our simulations, and for each phase of the ISM, we compute the mass-weighted metal fraction PDFs and fit $f(Z_i,M_i,S_i)$ to each. In fitting, we find the parameters $M_i$ and $S_i$ for each distribution that minimizes the distance between the CDF from the simulations and the CDF from the log-normal fit, or $D_{\rm CDF}$. For consistency, we use equally spaced bins in log of 0.05 dex in every case. 

%
% OLD TEXT - pre powerlaw
% We give the results of this fit in Figure~\ref{fig:log-normal} for the metal fraction PDFs of each element in the ISM as a whole. The PDFs computed directly from the simulation are shown in the top pane. Their log-normal best fits, and corresponding $D_{\rm CDF}$, are shown in the bottom panel. For clarity, we normalize each distribution by the peak (mode). In all cases, the metal mass fraction PDFs can be described with a log-normal distribution. This is a remarkable result. That the complex interplay between the dominant physical processes in the ISM, namely stellar feedback, turbulence, self-gravity, heating/cooling, and chemistry, produces metal distributions that are analytically tractable is surprising. Linking these instantaneous PDFs to a time evolution of the distributions parameters, $M_i$ and $S_i$, in a general way would create a powerful tool for modelling galactic chemical enrichment in semi-analytic models. This is beyond the scope of this work, but we discuss this further in Section~\ref{sec:discussion}.

% OLD TEXT ---- PRE powerlaw
%Upon closer inspection, we note that there are distinct differences with each metal and some noticeable deviations from true log-normality. Most elements have higher positive skew, tails towards higher metal fractions, than can be reasonable captured by a log-normal fit. Two elements in particular show quite broad distributions, N and Ba, which are less well fit by a log-normal as the other elements. As discussed earlier, these two elements have a distinct nucleosynthetic source, AGB winds, than the rest of the elements. Lower energy AGB winds generate broader enrichment distributions that are less well mixed than metals from core collapse and Type Ia supernovae. If this behavior is similar in star forming gas, we would expect elements from AGB winds to exhibit larger scatter in observed abundances. However, we note again that this figure represents all gas in the ISM. These deviations from log-normality and the high metallicity tails could be driven simply by grouping together qualitatively distinct phases of gas in the ISM, each with their own enrichment history. We investigate the metal fraction PDFs of individual gas phases below.

\begin{figure*}
\centering
\includegraphics[width=0.95\linewidth]{DD0390_element_by_element.png}
\caption{The numerical PDFs (solid histograms) and the associated log-normal + power-law fits (dashed lines) for a subset of the elements tracked in our simulation in each of the four gas phases defined in Section~\ref{sec:ejection}. For clarity, each distribution is normalized to the mode of the full-disk PDF (black) and is centered on the median value of the full-disk PDF. We note the vertical axis normalization is such that integrating over the shown PDF gives the mass fraction of that phase in the disk.}
\label{fig:log-normal}
\end{figure*}

%\begin{figure*}
%\centering
%\includegraphics[width=0.95\linewidth]{lognorm_elements.png}
%\caption{The metal mass fraction PDF's binned directly from the simulation (top) for all 15 elements tracked in the simulation and the associated log-normal fits (bottom) along with the minimized CDF distance for each. Given the wide range in mass fractions among species, we normalize the PDFs by their peak for clarity. These PDFs are shown at 350~Myr into the 500~Myr simulation; the choice of time was arbitrary.}
%\label{fig:log-normal}
%\end{figure*}

% NOTES:
%    Discuss how generally good of fits these are but how some are less good. Mention how N and Ba are less well mixed (reference above) and how this should really be examined in sub-groups of the ISM, motivating the next section. Next section will plot mean and std of fits and distributions as function of time for each phase, along with chi-squared good-ness of fits values. Need to find a way to estimate / compute the uncertainty for this!! Poisson in each bin? shift bins a little and estimate variance this way?

%   Give the distributions of metals at ONE point in time in each phase for two elements, O and Ba. Show the log fit and the histogram on each (maybe use orange and blue?). Discuss how the distributions are well fit by lognormals and the evolution of each, giving values for the fits and using dashed lines showing mean/std of full distribution when not well fit by a lognormal. 

\subsubsection{PDF Variation Across Gas Phase}
\label{sec:phase-pdfs}

%\subsubsection{Log-normality in each Phase of the ISM}
%\label{sec:phase-pdfs}
...
Unsurprisingly the diffuse HIM contains the most metal enriched gas, as it is comprised predominately of metal enriched supernova ejecta. Clearly this leads to long power-law tails towards high metal fractions for each species, with a very narrow, poorly defined peak at low metal fractions. Broadly, in colder gas the metal PDFs become less enriched with broader low metal fraction components and steeper, power-law tails. Although the extended power-law tail of the HIM leads to a large range of metal fractions, the HIM represents very little mass, and this tail represents recent, un-mixed enrichment. The comparatively narrow width in the log-normal regimes of the molecular phase and CNM is perhaps surprising. Although this gas is at lower metal fraction than the WNM and WIM, it would appear that it is more well mixed. This runs counter to the idea that mixing times should be long in colder gas unless mixing first proceeds rapidly in the hot phases before mixing in with cold, disconnected structures across the galaxy. 

Although these trends across phases hold for all elements, there are qualitative differences between elements, particularly between those ejected predominantly by AGB winds (e.g. Ba and N) and those ejected by supernovae (e.g. O and Mg). In all phases, except the HIM, the AGB-wind elements have broader distributions that are less well described by our adopted $p(Z)$ than the metals dominated by supernova enrichment. The power-law component of N and Ba is generally shallower than all other metals across each phase (except the HIM), particularly in the molecular phase and the CNM. Ba and N do not show significant differences among the rest of the metals in the HIM, though this is likely because the Ba and N present in this phase is dominated by the Ba and N produced in supernovae yields. Again these differences between yield sources could be driven both by differences in their enrichment timescales and therefore differences in the typical ISM environment each event encounters, and the differences in energetics between AGB winds and supernovae. AGB wind elements enrich the WNM and WIM directly, rather than the HIM, which would increase their mixing timescales and broaden their PDFs. 


%\textit{ It is unlikely that metals evolve uniformly across the large variations in density (10$^{-3}$ cm$^{-3}$ $<~n~<$ 10$^{3}$~cm$^{-3}$) and temperatures (10~K~$<~T~<$ 10$^{7}$~K) in our galaxy's ISM. Higher temperature gas has a higher sound speed and shorter mixing timescales than cold, dense star forming gas. From this, one would expect that high temperature gas is more well mixed than the disconnected dense clouds that comprise the galaxy's cold ISM. In addition, gas in the hot phase is generated almost entirely from HII regions and supernovae remnants and should therefore be more metal rich on average than cold gas. We investigate both of these expectations below. To simplify this discussion, we focus only on O and Ba as representative examples of elements produced predominately through supernovae explosions (core collapse or Type Ia) and elements produced in lower energy AGB winds respectively.}

%The previous section demonstrated that metals are log-normally distributed within the ISM of our dwarf galaxy as a whole, but given the significant variation in densities (10$^{-3}$cm$^{-3}$~$<~n~<~10^{3}$~cm$^{-3}$) and temperatures (10~K~$<~T~<$ 10$^{7}$~K) within the ISM, we investigate how this may vary across regimes in the ISM. (give reference to sources which show log-normal within different temperature regimes). In particular, turbulent mixing and log-normal density distributions seen in local ISM simulations do depend on the temperature of the examined gas. Higher temperature gas has a higher sound speed and shorter mixing timescales than cold, dense star forming gas. This should impact the variance of each distribution. To simplify this discussion, we focus on two elements only, O and N, as representative examples of elements whose injection is dominated by supernovae or stellar winds, respectively. Conclusions reached as to the general trends for these two elements can be generalized to other elements of similar injection source. 

%\textit{In Figure~\ref{fig:log-normal phases} we present the numerical PDFs (top, histograms) along with their log-normal fits (bottom, curves) for O (left) and Ba (right) in each of the five phases of our dwarf galaxy's ISM. It is clear that each phase can be described with a different log-normal distribution, and it is the sum of these distributions that drives the deviations from log-normality in the PDFs of the galaxy's ISM as a whole in Figure~\ref{fig:log-normal}. In both cases, the HIM deviates the most from a log-normal, with large tails towards higher metallicities. In O, these tails become progressively less extreme in the colder phases, with both the CNM and molecular gas conforming well to a log-normal distribution with the least amount of variance. Although it is expected that the hot phases are more metal enriched than the cooler phases, it is surprising that the cold, dense gas is more well-mixed than the hot phases. We discuss the implications of this result further in Section~\ref{sec:statistics}. In contrast with O, the PDFs in Ba appear to universally be less well described by a log-normal in all phases, with less distinct enrichment between the phases. Although the cold phases here are still less enriched than the hot phases, each still exhibits longer tails towards high metal fractions that are the source of the deviations from log-normal. The differences in the PDFs across elements from supernovae explosions and AGB winds indicates that these species diffuse and mix differently throughout the ISM. Again, AGB produced metals should show larger abundance variations observationally than those produced in supernovae. }

%\begin{figure}
%\centering
%\includegraphics[width=0.95\linewidth]{lognorm_O_Ba_phases}
%\caption{The metal mass fraction PDFs computed directly from the simulation (top) for O (left) and Ba (right) in each of the five phases of our galaxy's ISM at the same time as shown in Figure~\ref{fig:log-normal}. The log-normal best fits to each of these phases are given in the bottom row.}
%\label{fig:log-normal phases}
%\end{figure}

\subsubsection{The Time Evolution of Metal PDFs}
\label{sec:statistics}
We focus on the time evolution of the full numerical PDFs in this section so that our results here do not depend upon the choice of functional form for $p(Z)$. Figure~\ref{fig:phase-statistics} shows the evolution of three different statistics for the O (top) and Ba (bottom) PDFs. These two elements are treated as representative elements for supernova and AGB wind production respectively. The difference between the mean and median masses of the PDF is a measure of the skew of the PDF. Higher values indicate that metals are preferentially sequestered in metal-rich gas, and are less well mixed throughout the given phase. The skew is always positive in these distributions.

\begin{figure*}
\centering
\includegraphics[width=0.95\linewidth]{oxygen_barium_phases_CDF_evolution.png}
...
For O, the median mass fraction is ordered by phase temperature. The HIM is significantly more enriched than the cooler phases by anywhere from 0.1 dex to 2.5 dex, fluctuating by $\sim$ 1 dex over the simulation time. The frequent large skew in the HIM (see middle panel) and spread in the HIM (right panel), coupled with its continual fluctuation, suggests that the HIM is not in equilibrium. 
...
Each cooler phase in O is progressively less enriched (lower median), with smaller skew and spread; the two coldest phases, the CNM and molecular gas, have similar spread throughout the evolution, but a continuous $\sim$0.2 dex offset in median metal fraction. The offset between phases and increasingly well mixed gas from hot to cold indicates that metal enrichment in the ISM of galaxies proceeds first through mixing on large scales in a hot phase, before progressively cooling through multiple phases until enriching star forming gas. This is the only explanation for how the cold gas can rapidly homogenize over the whole galaxy within $\sim$50 Myr, roughly when the cold phase exhibits a nearly constant spread (middle and right panels). Individual enrichment events will have significantly higher metallicities than the ambient ISM in any phase, and will drive an increase in the difference between the mean and median of the PDF. These can be seen as the obvious spikes in the HIM and WIM. For O, the lack of these spikes in the two cold phases suggests again that enrichment does not occur directly in these phases, but proceeds more gradually through the warmer phases first.
...
Unlike O, the spread and skew in Ba for the CNM, WNM, and WIM \textit{increase} in during the evolution, reaching differences in 90$^{\rm th}$ and 10$^{\rm th}$ percentiles of over 1 dex in the CNM and over 2 dex in the WNM and WIM. The HIM is seemingly unaffected by this trend, and generally decreases in spread and skew over time. Given their lower energies, AGB winds more directly enrich the WNM and WIM, not the HIM as in supernovae. The consequence of this is clear in Figure~\ref{fig:log-normal} by the wider PDFs and longer power-law tails in Ba in these phases. These tails represent the most recently enriched gas, and is clearly much more locally confined than O. 

%{\bf (move to discussion?)}: 
The positive skew in all of the PDFs presented here implies that most of the mass of the galaxy has a metal fraction below what one would normally adopt as the average metallicity (i.e. the ratio between the total mass of metals and the total mass). Assuming star forming gas follows the same properties as the molecular gas, this distinction is small ($\sim$ 0.2 dex), but significant, for elements released during SNe explosions, and can be very significant, up to $\sim$ 0.8 dex, for elements released in AGB winds. Chemical evolution models, especially one-zone models, follow the mean metal fraction, not median. These results indicate that these models are biased to overestimate gas and stellar elemental abundances.
...
%We discuss possible caveats to this in Section~\ref{sec:caveats}. Does this extend to more massive? how does DM potential affect this? Are general trends still true in much larger galaxies? or are things very different once HIM crossing times are more like 100 Myr (aka MW size galaxies).
...
Unfortunately, there is insufficient star forming gas at any one time in these simulations to construct a PDF of metal distributions. However, star forming gas originates from the CNM, so it is reasonable to expect that this gas, and therefore stars themselves, follow a similar PDF to the CNM. To very this, Figure ~\ref{fig:stars} shows the difference between the mass fraction of a given element in stars and the median mass fraction from the CNM PDF at the time that particle formed. Stars seem to be sampled evenly around the median of the CNM distribution (i.e. there is no obvious bias towards high metal fractions). Additionally, if we further subdivide the CNM by density, the metal fraction PDFs narrow and tend towards the median value as a function of increasing density. High density gas, from which star formation occurs, is not biased towards higher metallicity in cold gas.
...
\item Metal distributions exhibit positive skew, such that the median metal fraction can be anywhere from 0.1 to 1.0 dex above the mean metal fraction. One-zone chemical evolution models which follow the mean abundance are likely biased to higher enrichment values.
...
\begin{figure*}
\centering
\includegraphics[width=0.95\linewidth]{O_Ba_spread_resolution-comparison.png}
