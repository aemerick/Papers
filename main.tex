...
\caption{ \textbf{need to adjust overlapping labels} Edge-on views of our dwarf galaxy at four different times in its evolution, separated by 100~Myr, beginning with the time at first star formation and ending at 300 Myr. Shown are density weighted projections of number density (top row), temperature slices (second row), and integrated projections giving \ion{H}{1} column density (third row) and O column density (fourth row). Each individual main sequence star particle is shown in the number density projections as a single black dot.  \textbf{use H2 in bottom panel instead}}
\label{fig:panel_x}
\end{figure*}
...
\caption{Same as Figure~\ref{fig:panel_x}, but showing face-on projections/slices. \textbf{use H2 in bottom panel instead}}
\label{fig:panel_z}
\end{figure*}
...
\caption{The 30~Myr time average gas disk scale height as a function of radius at various times throughout the simulation.}
...
These plots are useful to understand the time-varying properties of the ISRF in a spatially averaged sense. However, the local variations for individual regions can be much larger than is illustrated in these profiles. In Figure~\ref{fig:ISRF_2D} we present the full distribution of $G_{\rm o}$ at the end of the simulation time. This more effectively captures the large spikes in local ISRF from young, massive stars. The azimuthally averaged profiles capture the averaged heating and ionizing effects of the ISRF in our galaxy. This helps reduce star formation by increasing the thermal pressure support and equilibrium temperature of the ISM. However, it is the spikes that act as important local feedback to shut off star formation in dense, collapsing regions that satisfy our star formation criteria.
...
The total mass outflow rates and mass loading factors at 0.1, 0.25, 0.5, and 1.0 $R_{\rm vir}$ are shown in Figure~\ref{fig:mass_outflow}. We use the 10 Myr averaged SFR to compute the mass loading rates. The outflow rate at 0.1 $R_{\rm vir}$ is high, corresponding to mass loading factors on order of 50--100 throughout the simulation time. This quickly declines towards larger radii, however, with \textbf{check this effectively zero outflow past the virial radius}. \citet{Muratov2015} finds typical mass loading factors at 0.25 R$_{\rm vir}$ on order of 20--40 for galaxies with $v_{c} = 30$ km s$^{-1}$ at low redshift. Our dwarf galaxy exhibits mass loading factors between 4 and 40 at this radius; this is consistent with the results from \citet{Muratov2015}, but perhaps slightly below their typical values. 
...
\textbf{AE: Took plot and discussion of outflow metallicities out and replaced with metal mass loading factor (above)}
...
\textbf{Commented out all previous discussion of species abundance retention differences and stellar abundances to save for a future paper. Instead, here give a panel plot showing the column densities of the metals at various times?}

Dwarf galaxies efficiently, and preferentially, eject metals released in stellar feedback from their shallow potential wells \citep{MacLowFerrara1999,FerraraTolstoy2000}. This has been better quantified recently both observationally \citep[e.g.][]{Kirby2011-metals,Zahid2012,Peeples2014,McQuinn2015} and with more detailed cosmological simulations \citep{Angles-Alcazar2017,Muratov2017}. The right hand panel of Figure~\ref{fig:mass_outflow} gives the metal mass loading factor over time for our dwarf galaxy, considering only the new metals produced by the star particles. Metal mass loading factor is defined as 
\begin{equation}
\eta_{\rm metal} = \frac{\dot{M_{\rm metal}}}{\rm{SFR} \times \frac{M_{\rm metal}}{M_{*}}},
\end{equation}
where $\dot{M_{\rm metal}}$ is the metal mass outflow rate, $M_{\rm metal}$ is the total mass in metals produced, and $M_{*}$ is the total mass in stars.  \textbf{compare mass loading here and in other works... check back to Muratov for these. also cite miao and drummond.} \citep{Li2017}
...
\begin{figure*}
\centering
\includegraphics[width=0.23\linewidth]{metal_mass_loading} 
\includegraphics[width=0.23\linewidth]{metal_fraction_evolution}
\includegraphics[width=0.23\linewidth]{metal_mass_loading} 
\includegraphics[width=0.23\linewidth]{metal_fraction_evolution}
\caption{Full-box projections of the entire computational domain showing the total column density in each of 4 different metal species at the end of the simulation time. For a more reasonable comparison across metal fields with significant differences total column density, the maximum of the color bar scales are each set to the peak column density for that field, and the minimum to a fixed fraction of this maximum.}
\label{fig:metal_column_panels}
\end{figure*}
...
\textbf{Definitely a question that comes up a lot whenever I say our simulations are star-by-star and detailed (as in ``but what about binary stars''?). Do we need to discuss this here as a quick paragraph about how it would affect feedback properties (namely radiation and different stellar lifetimes)?} \citep{Ma2016-binary} go through this paper and check sources. discuss here briefly and include possible effects on yields

Most (word) models, including this work, of feedback and chemical evolution in galaxies ignore the effects of binary star evolution, generating feedback and yields from stellar populations that are comprised of many single stars. There are two effects that could significantly influence galaxy evolution: 1) radiation, and 2) runaway stars. The former could greatly increase the impact radiation feedback has on the galaxy, but its exact role is unclear since the photons emitted durign the extended lifetime are more likely to escape the galaxy (check this). The latter, however, could influence the typical regions where supernovae occur, as stars can travel appreciably far from their birth cloud (distance) before exploding, changing the effect of feedback and (yields). Since we model stars on a star-by-star basis, both of these effects could be reasonably accounted for by stochasitcally assigning binary star properties to some subset of our individual stars. This is beyond the scope of this project, but will be investigated in future works.
