...
\title{Simulating an Isolated Dwarf Galaxy with Individual Star Feedback and Chemistry}
...
In order to better understand the relationship between feedback and galactic chemical evolution, we have developed a new model for stellar feedback at high resolution ($\sim$ few pc) in global disk simulations using the adaptive mesh refinement hydrodynamics code \textsc{Enzo}. For the first time in galaxy scale simulations, we simulate detailed stellar feedback from individual stars including %stellar winds from massive and asymptotic giant branch stars,
AGB winds, 
photoelectric heating, Lyman-Werner dissociation radiation, ionizing radiation tracked through an adaptive ray-tracing radiative transfer method, and core collapse and Type Ia supernovae. {\bf We additionally include stellar winds from massive stars, but reduce their velocity due to computational constraints.}
...
\textit{Software:} \textsc{Enzo} \citep{Enzo2014}, \textsc{yt} \citep{yt}, \textsc{Grackle} \citep{GrackleMethod}, \textsc{Cloudy} \citep{Cloudy2013}, matplotlib, astropy
